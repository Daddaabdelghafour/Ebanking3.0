...
public class AccountEventHandlerService {
    ...
    public void handleAccountCreationEvent(@NotNull AccountCreationEvent event) {
        try {
            log.info("========== HANDLING AccountCreationEvent for account ID: {} ==========", event.getId());
            ...
            log.info("Account saved to database: ID={}, AccountNumber={}", savedAccount.getId(), savedAccount.getAccountNumber());
            ...
            log.error("ERROR handling AccountCreationEvent for account {}: {}", event.getId(), e.getMessage(), e);
            throw e;
        }
    }
    ...
    public void handleAccountActivationEvent(@NotNull AccountActivatedEvent event) {
        try {
            ...
            Account account = accountRepository
                    ...
                    .orElseThrow(
                            () -> new AccountNotFoundException("Account not found with ID: " + event.getId())
                    );
            ...
            log.info("Account activated: ID={}, Status={}", activatedAccount.getId(), activatedAccount.getStatus());
            ...
            log.error("ERROR handling AccountActivatedEvent for account {}: {}", event.getId(), e.getMessage(), e);
            throw e;
        }
    }
    ...
    public void handleAccountSuspensionEvent(@NotNull AccountSuspendedEvent event) {
        try {
            log.info("========== HANDLING AccountSuspendedEvent for account ID: {} ==========", event.getId());
    ...
    public void handleAccountDeletedEvent(@NotNull AccountDeletedEvent event) {
        try {
            log.info("========== HANDLING AccountDeletedEvent for account ID: {} ==========", event.getId());
